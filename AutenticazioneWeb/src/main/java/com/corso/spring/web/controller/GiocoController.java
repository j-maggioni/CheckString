...
public class GiocoController {
	...
	public String salvaPunteggio(HttpSession session, @ModelAttribute("partita") @Valid GiocoVO partita,
		...
		System.out.println(partita.getGioco().getClass());
		...
		model.addAttribute("gioco_prec",partita.getGioco());

		List<GiocoVO> classificaGenerale = ConverterGiocoToGiocoVO.convertList(giocoService.getAllBest(
				partita.getGioco()));
	...
	private GiocoVO istanziaPartita(GiochiEnum gioco, HttpSession session){
		...
		UtenteVO utenteVO = (UtenteVO) session.getAttribute("utente");
		return new GiocoVO(dateFormat.format(currentDate),0, gioco.name(), utenteVO.getNome());
	}
